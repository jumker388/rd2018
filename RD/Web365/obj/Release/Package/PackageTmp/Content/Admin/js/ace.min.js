...
function widget_boxes() {
    $("body").delegate(".widget-toolbar > [data-action]", "click", function (k) {
        k.preventDefault();
        var j = $(this);
        var l = j.data("action");
...
function setLanguage() {
    ...
    if ($.cookie('.ASPL') != null && $.cookie('.ASPL') == '2') {

        $(document.body).addClass('skin-1');
        $(".ace-nav > li.grey").addClass("dark");

        $('#sl-chosen-language option[value="2"]').attr({
            selected: ''
        });

    } else {
        $('#sl-chosen-language option[value="1"]').attr({
            selected: ''
        });
    }

    $('.chzn-select').chosen({ width: "100%" }).change(function (e) {
        $.cookie('.ASPL', e.target.value, { expires: 30, path: '/' });
        location.reload();
    });
